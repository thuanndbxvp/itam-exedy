...
export default function EditCompanyForm({ company }: { company: Company }) {
  ...
  const [form, setForm] = useState({ name: company.name, notes: company.notes ?? '' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsPending(true)
    try {
      const res = await fetch(`/api/settings/companies/${company.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
  ...
  return (
    <div className="max-w-xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Sửa công ty</h1>
      </div>
