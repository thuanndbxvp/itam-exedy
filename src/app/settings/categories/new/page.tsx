...
export default function NewCategoryPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsPending(true)
    try {
      const res = await fetch('/api/settings/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-5">
          <div>
            ...
            <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              className="w-full bg-slate-50 border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition" placeholder="VD: Laptop, Màn hình" />
          </div>
          <div>
