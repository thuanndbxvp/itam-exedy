...
export default function EditUserForm({
  user, departments, customRoles, companies, locations, managers,
}: Props) {
  ...
  const [form, setForm] = useState({
    ...
    remote: user.remote,
    vip: user.vip,
    autoassignLicenses: user.autoassignLicenses,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.firstName.trim() || !form.email.trim()) {
      showCommandResult({ ok: false, code: 'VALIDATION', message: 'Vui lòng điền đầy đủ họ tên và email.' })
      return
    }
    setIsPending(true)
    try {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={form.username}
                ...
                className={inputCls} />
              <p className="mt-1 text-xs text-gray-500">Dùng để đăng nhập thay email. Phải là duy nhất.</p>
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <input type="text" value={form.avatar}
                ...
                placeholder="https://..."
                className={inputCls} />
              <p className="mt-1 text-xs text-gray-500">Upload ảnh thật sẽ làm ở Epic I (sau).</p>
            </div>
          </div>
        </fieldset>
...
/**
 * ToggleRow — toggle switch có label/desc, dùng cho các flag boolean
 * (activated, remote, vip, autoassignLicenses).
 */
function ToggleRow({
