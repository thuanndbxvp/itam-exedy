...
/**
 * LicenseDetailClient — tabbed view cho license detail.
 *
 * Tabs:
 *  - Tổng quan: thông tin license + bảng seats
 *  - Lịch sử: timeline các thao tác CHECKOUT/CHECKIN/UPDATE
 */
...
import LicenseHistoryTimeline from '@/components/licenses/LicenseHistoryTimeline'
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{license.name}</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                <div>
                  ...
                  {!isAdmin && license.productKey && (
                    <p className="text-xs text-amber-600 mt-1">
                      Product Key đã được ẩn. Liên hệ Admin để xem chi tiết.
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-900 flex items-center space-x-2">
                    ...
                    <span>{formatDate(license.expirationDate)}</span>
                    {isExpired && (
                        ...
                        return (
                          ...
                          >
                              ...
                              {seat.assignedUser ? (
                                <div className="flex items-center space-x-2">
                                  <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-bold">
                                    {seat.assignedUser.firstName?.charAt(0).toUpperCase() ?? '?'}
                                  </div>
                                  <span>
                                    {seat.assignedUser.firstName}
                                    {seat.assignedUser.lastName ? ' ' + seat.assignedUser.lastName : ''}
                                  </span>
                                </div>
                              ) : seat.assignedAsset ? (
                                <div className="flex items-center space-x-2">
                            ...
                            <td className="px-6 py-4 text-gray-500 text-sm">
                              {seat.notes || <span className="text-gray-400 italic">---</span>}
                            </td>
        ...
        {activeTab === 'history' && (
          <div className="p-6">
            <LicenseHistoryTimeline licenseId={license.id} />
          </div>
        )}
      </div>
    </div>
