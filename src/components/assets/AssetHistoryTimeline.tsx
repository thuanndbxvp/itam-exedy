...
/**
 * AssetHistoryTimeline — tab Lịch sử cấp phát.
 *
 * Fetch /api/assets/[id]/history, render timeline lịch sử thao tác:
 *   - CHECKOUT (cấp phát) → xanh dương
 *   - CHECKIN  (thu hồi) → xám
 *   - UPDATE   (cập nhật thông tin) → vàng
 *   - CREATE / DELETE / AUDIT / NOTE_ADDED → xám nhạt
 *
 * Hiển thị: icon, action, actor (firstName lastName), oldValues→newValues diff, notes, timestamp.
 */
...
interface HistoryEvent {
  id: string
  actionType: 'CREATE' | 'UPDATE' | 'CHECKOUT' | 'CHECKIN' | 'AUDIT' | 'DELETE' | 'RESTORE' | 'NOTE_ADDED' | 'ACCEPTED' | 'DECLINED'
  notes: string | null
  ...
  if (events.length === 0) {
    return (
      <div className="p-12 text-center text-gray-500">
        ...
        Chưa có lịch sử thao tác nào trên tài sản này.
      </div>
    )
  }
